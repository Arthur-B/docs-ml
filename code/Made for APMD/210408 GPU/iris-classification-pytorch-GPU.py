...
Same as code from previous class but this time adapted to GPU
...
X_train = torch.tensor(X_train).float().to(device)  # send the tensor to GPU
...
def train(model, loss_fn, optimizer, X_train, X_test, y_train, y_test,
    ...
    for i in range(epochs):
        y_pred = model(X_train)             # Make prediction
        loss = loss_fn(y_pred, y_train)     # Determine loss

        optimizer.zero_grad()   # Put gradient to 0
        loss.backward()         # Back propagate the gradients based on loss
        optimizer.step()        # Adjust the weights following the optimizer
